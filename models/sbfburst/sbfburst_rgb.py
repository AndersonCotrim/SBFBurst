...
class SBFBurstRGB(nn.Module):
    def __init__(self, mid_channels=64, encoder_blocks=40):
        ...
        # preencoder module
        self.preencoder = ResidualBlocksWithInputConv(3, mid_channels, 5)
        ...
        self.skippers_up = nn.Sequential(PixelShufflePack(3, mid_channels, 2, upsample_kernel=3),
                                         PixelShufflePack(mid_channels, mid_channels, 2, upsample_kernel=3))
    ...
    def compute_flow(self, input_burst):
        ...
        other_images = input_burst[:, 1:, :, :, :].reshape(-1, num_channels, height, width)
        ref_images = input_burst[:, :1, :, :, :].repeat(1, num_images - 1, 1, 1, 1).reshape(-1, num_channels,
                                                                                            height, width)
...
if __name__ == '__main__':
    input_tensor = torch.randn(2, 8, 3, 48, 48).to('cuda')
    model = SBFBurstRGB()
    model = model.to('cuda')
